...
public class MainMenuManager : MonoBehaviour
{
    ...
    // this function inside MainMenuManager
   public void TryLogin()
    {
        string email = emailInput.text;
        string password = passwordInput.text;

        Debug.Log("Attempting to connect to DB...");

        DBUpdater.Instance.Login(email, password, (success, message) => 
        {
            if (success)
            {
                Debug.Log("SUCCESS! Logged in as: " + DBUpdater.Instance.currentUser.name);
                
                // 1. Fade out the login screen smoothly
                FadeOut(loginCanvas);
                
                // 2. Call your built-in function to show the Main Menu smoothly
                ShowMainMenu();
            }
            else
            {
    ...
    // Link this to your "Guest" button in the Unity Inspector!
    /*public void ContinueAsGuest()
    {
        DBUpdater.Instance.LoginAsGuest();
        
        // Do the exact same smooth transition as a real login
        FadeOut(loginCanvas);
        ShowMainMenu();
    }*/
    ...
    private void ShowMainMenu()
    {
        ...
        FadeIn(moduleCanvas);
        FadeIn(userCanvas);
        if (!hasPlayedIntro && audioSource != null && introClip != null)
        {
